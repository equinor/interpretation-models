...
def map_dataobject_datatype(ow_datatype: OWDataType) -> DataType:
    mapping = {
        OWDataType.MAP2D: DataType.SURFACE_GRID,
        OWDataType.RGRID: DataType.SURFACE_GRID,
        OWDataType.HORIZON3D: DataType.HORIZON,
        OWDataType.POLYGON_SET: DataType.POLYGON,
        OWDataType.POINT_SET: DataType.POINT_SET,
    }
    return mapping.get(ow_datatype, DataType.OTHERS)
...
def map_collection_item(
    ...
    source_metadata = SourceMetadata(
        ...
        database=source_context.database,
        project=source_context.project,
        id=id_generate(source_context, str(ow_data_object.data_object_id)) if ow_data_object.data_object_id is not None else None,
    ...
    source_ow_metadata = OWCollectionItemMetadata(
        ...
        data_object_id=str(ow_data_object.data_object_id) if ow_data_object.data_object_id is not None else None,
